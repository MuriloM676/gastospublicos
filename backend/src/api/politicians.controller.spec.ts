...
describe('PoliticiansController', () => {
  ...
  beforeEach(async () => {
    const mockPrisma = {
      politician: { findUnique: jest.fn(), findMany: jest.fn(), count: jest.fn() },
  ...
  describe('getPolitician', () => {
    it('should return politician profile with rankings', async () => {
      ...
      prisma.politician.findMany.mockResolvedValue(allPoliticiansForRanking as any);

      const result = await controller.getPolitician('1');

      expect(result).toEqual({
        id: 1, name: 'João Silva', photoUrl: 'https://foto.com/joao.jpg',
        party: 'PT', state: 'SP', position: 'Deputado Federal',
        firstElectionDate: '2018-01-01T00:00:00.000Z',
        totalExpenses: 3000, monthlyAverage: expect.any(Number),
        nationalRanking: 2, stateRanking: 1, partyRanking: 1,
      });
    });
    ...
    it('should throw NotFoundException when politician does not exist', async () => {
      ...
      await expect(controller.getPolitician('999')).rejects.toThrow(NotFoundException);
    });

    it('should handle politician with no expenses', async () => {
      const noExpensePolitician = { ...mockPolitician, expenses: [] };
      prisma.politician.findUnique.mockResolvedValue(noExpensePolitician as any);
      prisma.politician.findMany.mockResolvedValue(allPoliticiansForRanking as any);
    ...
    it('should handle null firstElectionDate', async () => {
      const noElectionPolitician = { ...mockPolitician, firstElectionDate: null };
      prisma.politician.findUnique.mockResolvedValue(noElectionPolitician as any);
      prisma.politician.findMany.mockResolvedValue(allPoliticiansForRanking as any);
  ...
  describe('getExpenses', () => {
    const mockExpenses = [
      { id: 1, expenseDate: new Date('2024-03-15'), category: { id: 1, name: 'Combustível' }, supplier: 'Posto ABC', description: 'Combustível', amount: 500, documentUrl: 'https://doc.com/1' },
      { id: 2, expenseDate: new Date('2024-01-10'), category: { id: 2, name: 'Passagem' }, supplier: 'Latam', description: 'Passagem aérea', amount: 1500, documentUrl: 'https://doc.com/2' },
    ];
    ...
    it('should return paginated expenses without filters', async () => {
      ...
      expect(result).toEqual({
        items: [
          { date: '2024-03-15', category: 'Combustível', supplier: 'Posto ABC', description: 'Combustível', amount: 500, documentUrl: 'https://doc.com/1' },
          { date: '2024-01-10', category: 'Passagem', supplier: 'Latam', description: 'Passagem aérea', amount: 1500, documentUrl: 'https://doc.com/2' },
        ],
    ...
    it('should apply year filter when provided', async () => {
      ...
      expect(prisma.expense.count).toHaveBeenCalledWith({
        where: { politicianId: 1, expenseDate: { gte: new Date('2024-01-01'), lte: new Date('2024-12-31') } },
      });
    });

    it('should apply category filter when valid category name is provided', async () => {
      prisma.expenseCategory.findUnique.mockResolvedValue({ id: 5, name: 'Combustível' } as any);
      prisma.expense.count.mockResolvedValue(1);
      prisma.expense.findMany.mockResolvedValue([mockExpenses[0]] as any);

      await controller.getExpenses('1', undefined, 'Combustível');

      expect(prisma.expenseCategory.findUnique).toHaveBeenCalledWith({ where: { name: 'Combustível' } });
      expect(prisma.expense.count).toHaveBeenCalledWith({ where: { politicianId: 1, categoryId: 5 } });
    });
    ...
    it('should not apply category filter when category name is not found', async () => {
      ...
      expect(prisma.expense.count).toHaveBeenCalledWith({ where: { politicianId: 1 } });
    });
    ...
    it('should apply pagination with custom page and limit', async () => {
      ...
      expect(prisma.expense.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 10, take: 10 }));
    });
    ...
    it('should cap limit at 100', async () => {
      ...
      expect(prisma.expense.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 0, take: 100 }));
    });
  });
  ...
  describe('getEvolution', () => {
    it('should return monthly and yearly evolution', async () => {
      ...
      expect(result).toEqual({
        monthly: [{ period: '2024-01', total: 3000 }, { period: '2024-06', total: 1500 }],
